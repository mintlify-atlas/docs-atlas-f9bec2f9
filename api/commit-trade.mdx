...
`WalletPublicService` validates all price arguments before fetching the market price or touching the exchange. Any violation throws an `Error` with a descriptive message:

| Rule | Error message |
|---|---|
| `takeProfitPrice` is not a finite number | `"take-profit price must be a valid number"` |
| `stopLossPrice` is not a finite number | `"stop-loss price must be a valid number"` |
| `stopLossPrice >= averagePrice` | `"stop-loss price must be less than average price"` |
| `takeProfitPrice <= averagePrice` | `"take-profit price must be greater than average price"` |
...
1. `WalletPublicService` fetches the current average price, then validates all four price arguments against the rules above.
