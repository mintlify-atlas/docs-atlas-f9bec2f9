...
**Caching:** Results are cached for **30 seconds** per symbol via a `ttl()` wrapper on the underlying price ticker.
...
Returns the total USDT balance for the account — free plus locked — as a `number`. The `symbol` argument is accepted for API consistency with the per-symbol queue, but the USDT balance is account-wide; passing any symbol returns the same value.
...
<Note>
**Cache TTLs.** `fetchOrders` and `fetchPnl` are cached for 10 minutes per `symbol + limit` key. `fetchPrice` uses a 30-second cache on the underlying ticker. After a `commit*` operation, `walletPublicService` automatically calls `walletPrivateService.clear()` to flush the orders and PnL caches, so the next read reflects the post-trade state. To flush the cache manually without placing a trade, call `commitReload(symbol)` — or call `wallet.walletPrivateService.clear()` directly to flush all symbols at once.
</Note>
