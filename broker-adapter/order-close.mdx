...
<Steps>
  ...
  <Step title="Step 2 — Verify loop">
    ...
    ```
    A separate verification pass polls `openOrders` up to 10 times to confirm the book is truly empty before the sell is placed. This guards against edge cases where a cancel appears to succeed but the order lingers briefly in the exchange's state. If orders remain after all verification rounds, the function throws — the engine retries the entire close sequence on the next tick.
  </Step>
